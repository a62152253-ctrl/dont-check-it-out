...
export function VaultSetup({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (masterPassword.length < 8) {
      setLocalError(
        language === "PL"
          ? "Hasło główne musi mieć co najmniej 8 znaków."
  ...
  return (
    <div className="relative max-w-xl mx-auto bg-[#0a0a0a]/90 backdrop-blur-md border border-white/10 rounded-2xl p-8 space-y-6 shadow-2xl relative overflow-hidden my-8 cyber-scanner">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-[10px] font-mono uppercase text-slate-500 mb-1.5 px-0.5 tracking-wider font-semibold">
            {t.masterPassLabel}
          </label>
          <input
            type="password"
        ...
        {masterPassword && (
          <div className="bg-[#121212]/50 border border-white/5 p-3.5 rounded-xl space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded ${masterPasswordStrength.color}`}
              >
            ...
            <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-300 ${
                  masterPasswordStrength.score <= 2
                    ? "bg-rose-500"
                    : masterPasswordStrength.score <= 4
                      ? "bg-amber-500"
                      : "bg-emerald-500"
                }`}
                style={{
                  width: `${(masterPasswordStrength.score / 5) * 100}%`,
                }}
              />
            </div>
          </div>
      ...
      <div className="border-t border-white/5 pt-4">
        ...
        >
          ...
          <span>
            {showConfigHint
              ? "Ukryj szczegóły architektury"
              : "Pokaż szczegóły architektury"}
          </span>
        </button>
        ...
        {showConfigHint && (
          <p className="text-[10px] font-mono text-slate-500 mt-2.5 leading-relaxed bg-[#121212]/30 p-3 rounded-lg border border-white/5">
            ...
            * Przeiterujemy hasło 100 000 razy za pomocą PBKDF2-HMAC-SHA256, aby
            uzyskać KEK (Key Encryption Key). <br />
            * Tworzymy losowy 256-bitowy Vault Key, który szyfrujemy KEK za
            pomocą AES-GCM. <br />* Ten zaszyfrowany blob synchronizujemy z
            Firebase, abyś miał dostęp na innych urządzeniach bez ryzyka wycieku
            hasła.
          </p>
        )}
      </div>
