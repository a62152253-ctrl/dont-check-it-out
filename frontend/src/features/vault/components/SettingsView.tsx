...
import type { AccentThemeName } from "../hooks/useVaultTheme";
    ...
    <div className="relative lg:col-span-9 space-y-6">
      ...
      <div className="bg-[#0c0c0c] border border-white/5 rounded-lg p-6 space-y-6">
        <div className="flex items-center gap-2 border-b border-white/5 pb-3">
          ...
          <h3 className="text-sm font-semibold uppercase text-slate-300 font-mono">
            Ustawienia Sejfu / Security Settings
          </h3>
        </div>
        ...
        <div className="space-y-2">
          <label className="block text-xs font-mono uppercase text-slate-400 font-bold">
            Czas bezczynności do blokady sejfu (Auto-lock)
          </label>
          <p className="text-[10px] font-mono text-slate-500 leading-normal">
            Dla bezpieczeństwa, jeśli nie wykażesz aktywności w oknie, klucze
            sesji zostaną usunięte z lokalnej pamięci RAM.
          </p>
          <select
          ...
          <div className="flex flex-wrap gap-2.5 pt-1">
            {(
              ...
              return (
                ...
                >
                  <span
                    className={`w-2.5 h-2.5 rounded-full ${themeObj.color}`}
                  />
        ...
        <div className="space-y-3 pt-4 border-t border-white/5">
          ...
          <div className="p-4 bg-[#121212] border border-white/5 rounded-lg space-y-3 max-w-md">
            <h5 className="text-xs font-semibold text-white">
              Eksportuj Sejf (Zaszyfrowany JSON)
            </h5>
            <p className="text-[10px] font-mono text-slate-500">
              Pobierz kopię zapasową wszystkich wpisów w bezpiecznym formacie.
            </p>
            <button
          ...
          <div className="p-3 bg-[#121212] border border-white/5 rounded-lg flex items-center justify-between text-xs font-mono">
            <div className="space-y-1">
              <span className="flex items-center gap-2 text-emerald-400 font-bold">
                ...
                {isSandbox
                  ? "Lokalny Sejf Sandbox (Offline)"
                  : "Wszędzie bezpieczny (Synchronizowany)"}
              </span>
              <span className="text-[10px] text-slate-500 block">
                {isSandbox
                  ? "Lokalny magazyn LocalStorage"
                  : "Durable Cloud Provider: Firebase Firestore (Active)"}
              </span>
            </div>
        ...
        {/* Reset sejfu */}
        <div className="space-y-3 pt-4 border-t border-white/5">
          <label className="block text-xs font-mono uppercase text-rose-500 font-bold">
            Strefa Niebezpieczna (Danger Zone)
          </label>
          <p className="text-[10px] font-mono text-slate-500 leading-normal">
            Chcesz całkowicie skasować swoje hasła, klucze oraz konfigurację
            PBKDF2 sejfu, aby zacząć od nowa? Tej akcji nie można cofnąć.
          </p>
          <button
            onClick={resetVault}
