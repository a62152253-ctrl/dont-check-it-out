...
export function APIKeyDetails({ fields, id, name, copiedId, copiedField, copyText, handleDownloadDotenv }: Props) {
  return (
    <div className="space-y-3">
      <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative max-h-[160px] overflow-y-auto">
        ...
        <pre className="text-[10px] font-mono text-emerald-400 select-all whitespace-pre-wrap leading-relaxed mt-1 break-all pr-6">
          {fields.dotenvContent || "Brak danych .env"}
        </pre>
        <button
          onClick={() => copyText(fields.dotenvContent || "", id, "dotenv")}
          className="absolute right-2 top-2.5 text-slate-500 hover:text-white cursor-pointer"
        >
          {copiedId === id && copiedField === "dotenv" ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
        </button>
      </div>

      <button
        onClick={() => handleDownloadDotenv(name, fields.dotenvContent)}
