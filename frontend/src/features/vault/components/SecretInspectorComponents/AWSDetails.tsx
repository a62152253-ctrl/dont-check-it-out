...
export function AWSDetails({ fields, id, environment, isPassVisible, copiedId, copiedField, copyText, setVisiblePasswords, handleCopyAWSExports }: Props) {
  return (
    <div className="space-y-3">
      <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative">
        ...
        <button
          onClick={() => copyText(fields.awsAccessKeyId || "", id, "aws_id")}
          className="absolute right-2 top-3 text-slate-500 hover:text-white cursor-pointer"
          title="Kopiuj ID"
        >
          {copiedId === id && copiedField === "aws_id" ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
        </button>
      </div>
      ...
      <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative">
        ...
        <div className="absolute right-2 top-3 flex items-center gap-1.5 text-slate-500">
          ...
          <button
            onClick={() => copyText(fields.awsSecretAccessKey || "", id, "aws_secret")}
            className="hover:text-white cursor-pointer"
          >
            {copiedId === id && copiedField === "aws_secret" ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
          </button>
        </div>
      </div>
