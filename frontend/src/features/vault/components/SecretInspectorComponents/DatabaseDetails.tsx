...
export function DatabaseDetails({ fields, id, isPassVisible, setVisiblePasswords, handleCopyDbUri }: Props) {
  return (
    <div className="space-y-2.5">
      <div className="grid grid-cols-3 gap-1.5">
        <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-1">
          ...
          <span className="text-[11px] font-mono text-sky-400 font-bold">{fields.dbEngine || "PostgreSQL"}</span>
        </div>
        <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-2">
          ...
          <span className="text-[11px] font-mono text-slate-300 truncate block select-all">{fields.dbHost || "localhost"}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-1">
          ...
          <span className="text-[11px] font-mono text-slate-300">{fields.dbPort || "5432"}</span>
        </div>
        <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-2">
          <span className="block text-[8px] font-mono text-slate-500 uppercase">Nazwa Bazy</span>
          <span className="text-[11px] font-mono text-slate-300 truncate block select-all">{fields.dbName || "postgres"}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <div className="bg-[#121212] border border-white/5 p-2 rounded">
          ...
          <span className="text-[11px] font-mono text-slate-300 truncate block select-all">{fields.dbUser || "Brak"}</span>
        </div>
        <div className="bg-[#121212] border border-white/5 p-2 rounded relative">
          ...
          <span className="text-[11px] font-mono text-slate-300 truncate block">
            {isPassVisible ? fields.dbPassword : "••••••••••"}
          </span>
          <button
      ...
      <button
        onClick={() => handleCopyDbUri(fields)}
