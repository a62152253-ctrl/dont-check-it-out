  ...
  return (
    <div className="lg:col-span-9 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/85 p-5 rounded-2xl border border-white/10 shadow-xl flex flex-col justify-between hover:-translate-y-0.5 hover:border-white/15 transition-all duration-300 relative group overflow-hidden">
          ...
          <div>
            <span className="text-xs  font-bold uppercase tracking-wide text-slate-500">Total Secrets</span>
            <h3 className="text-3xl font-display font-semibold text-white mt-1.5">{stats.total}</h3>
          </div>
          <p className="text-[10px]  text-slate-500 mt-4">Wszystkie aktywne klucze i hasła</p>
        </div>

        <div className="bg-slate-900/85 p-5 rounded-2xl border border-white/10 shadow-xl flex flex-col justify-between hover:-translate-y-0.5 hover:border-blue-500/25 transition-all duration-300 relative group overflow-hidden">
          ...
          <div>
            <span className="text-xs  font-bold uppercase tracking-wide text-blue-400">Categories</span>
            <h3 className="text-3xl font-display font-semibold text-blue-400 mt-1.5">{stats.categoriesCount}</h3>
          </div>
          <p className="text-[10px]  text-slate-500 mt-4">Aktywne grupy i typy sekretów</p>
        </div>

        <div className="bg-slate-900/85 p-5 rounded-2xl border border-white/10 shadow-xl flex flex-col justify-between hover:-translate-y-0.5 hover:border-amber-500/25 transition-all duration-300 relative group overflow-hidden">
          ...
          <div>
            <span className="text-xs  font-bold uppercase tracking-wide text-amber-400">Recently Added</span>
            <h3 className="text-3xl font-display font-semibold text-amber-400 mt-1.5">{stats.recentlyAdded}</h3>
          </div>
          <p className="text-[10px]  text-slate-500 mt-4">Wpisy dodane w ciągu 30 dni</p>
        </div>

        <div className="bg-slate-900/85 p-5 rounded-2xl border border-white/10 shadow-xl flex flex-col justify-between hover:-translate-y-0.5 hover:border-rose-500/25 transition-all duration-300 relative group overflow-hidden">
          ...
          <div>
            <span className="text-xs  font-bold uppercase tracking-wide text-rose-400">Favorites</span>
            <h3 className="text-3xl font-display font-semibold text-rose-400 mt-1.5">{stats.favorites}</h3>
          </div>
          <p className="text-[10px]  text-slate-500 mt-4">Oznaczone jako ulubione dewelopera</p>
        </div>
      </div>
        ...
        <div className="md:col-span-7 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-2.5">
              ...
              <h4 className="text-xs font-bold uppercase tracking-wide text-slate-300">Recently Opened / Ostatnio używane</h4>
            </div>
              ...
              <div className="space-y-2">
                {recentlyOpenedSecrets.map(secret => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 bg-white/[0.01] border border-slate-800 hover:border-white/10 rounded-lg hover:bg-white/[0.03] transition-all text-left group cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h5 className="text-xs font-semibold text-white group-hover:text-indigo-400 transition-colors">{secret.name}</h5>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-2.5">
              ...
              <h4 className="text-xs font-bold uppercase tracking-wide text-slate-300">Security Overview / Audyt bezpieczeństwa</h4>
            </div>
            
            <div className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between items-baseline">
                  <span className="text-xs  font-bold text-slate-400">Security Status</span>
                ...
                {/* Visual Progress Bar */}
                <div className="flex items-center gap-1  text-xs tracking-tight">
                  <span className="text-slate-500 mr-1">Rating:</span>
                  {Array.from({ length: 10 }).map((_, i) => {
                    const isActive = i < Math.round(securityScore / 10);
                    return (
                      <span key={i} className={isActive ? (securityScore >= 80 ? "text-indigo-500" : securityScore >= 50 ? "text-amber-500" : "text-rose-500") : "text-slate-800"}>
                        █
                      </span>
                    );
                  })}
                ...
                <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden border border-slate-800">
                  <div 
                    className={`h-full transition-all duration-700 ${
                      securityScore >= 80 ? "bg-indigo-500" : securityScore >= 50 ? "bg-amber-500" : "bg-rose-500"
                    }`}
              ...
              <div className="space-y-2 pt-1">
                <div className="flex items-center justify-between text-xs  py-1.5 border-b border-slate-800">
                  <span className="flex items-center gap-2 text-slate-400">
                    <span className="text-indigo-500 font-bold">✓</span> Szyfrowany sejf lokalny (Encrypted Vault)
                  </span>
                  <span className="text-indigo-400 bg-indigo-500/10 border border-indigo-500/10 px-2 py-0.5 rounded text-[10px]">
                    Zabezpieczony
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between text-xs  py-1.5 border-b border-slate-800">
                  <span className="flex items-center gap-2 text-slate-400">
                    {stats.weak === 0 ? (
                      <>
                        <span className="text-indigo-500 font-bold">✓</span> Brak słabych haseł deweloperskich
                      </>
                    ) : (
                      <>
                ...
                <div className="flex items-center justify-between text-xs  py-1.5 border-b border-slate-800">
                  <span className="flex items-center gap-2 text-slate-400">
                    {duplicatedCount === 0 ? (
                      <>
                        <span className="text-indigo-500 font-bold">✓</span> Wszystkie hasła są unikalne
                      </>
                    ) : (
                      <>
                ...
                <div className="flex items-center justify-between text-xs  py-1.5">
                  <span className="flex items-center gap-2 text-slate-400">
                    {oldSecretsCount === 0 ? (
                      <>
                        <span className="text-indigo-500 font-bold">✓</span> Wszystkie hasła są świeże
                      </>
                    ) : (
                      <>
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    oldSecretsCount === 0 
                      ? "text-indigo-400 bg-indigo-500/10 border border-indigo-500/10"
                      : "text-slate-500 bg-slate-800 border border-slate-800"
                  }`}>
        ...
        <div className="md:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-2.5">
              ...
              <h4 className="text-xs font-bold uppercase tracking-wide text-slate-300">Quick Actions / Szybkie akcje</h4>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-4 px-4 bg-indigo-500 hover:bg-indigo-600 text-white font-bold text-sm rounded-lg transition-all shadow-lg hover:shadow-indigo-500/10 cursor-pointer text-center"
              >
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg transition-all border border-slate-800 gap-2 text-xs  cursor-pointer"
                >
              ...
              <button
                onClick={lockVault}
                className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400  text-xs uppercase border border-rose-500/20 rounded-lg transition-all cursor-pointer mt-1"
              >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-800 pb-2.5">
              ...
              <h4 className="text-xs font-bold uppercase tracking-wide text-slate-300">Vault Activity / Dziennik zdarzeń</h4>
            </div>
            ...
            {activityLogs.length === 0 ? (
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {activityLogs.slice(0, 7).map((log, index) => (
                  <div key={index} className="flex items-start justify-between text-xs border-b border-slate-800 pb-2.5 last:border-0 last:pb-0 ">
                    <div className="space-y-0.5">
                      <p className="text-white font-medium text-xs">{log.action}</p>
                      <p className="text-[10px] text-slate-500 break-all leading-normal">{log.detail}</p>
                    </div>
