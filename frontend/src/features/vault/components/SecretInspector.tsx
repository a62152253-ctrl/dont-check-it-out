  ...
  if (!secret) {
    return (
      <div className="lg:col-span-4">
        <div className="w-full bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-12 shadow-2xl text-center text-slate-500 font-mono text-xs flex flex-col items-center justify-center">
    ...
    <div className="lg:col-span-4">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6 space-y-5 shadow-2xl relative overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.02)]"
        >
          ...
          <div className="flex items-start justify-between gap-3 border-b border-white/10 pb-4">
            <div>
              <h3 className="font-display font-semibold text-xl text-white tracking-tight break-words drop-shadow-sm">{secret.name}</h3>
              <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
                <span className="text-[10px] font-mono font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-0.5 rounded-md uppercase tracking-wider">
                  {secret.category}
                </span>
                {secret.project && (
                  <span className="text-[10px] font-mono bg-white/5 text-slate-300 px-2.5 py-0.5 rounded-md border border-white/10">
                    {secret.project}
                  </span>
                )}
              </div>
