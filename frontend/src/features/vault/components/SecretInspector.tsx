...
import type { DeveloperFields, DecryptedSecret } from "../types";

// Helper components for categories

function AwsCategoryDetails({ secret, isPassVisible, setVisiblePasswords, copiedId, copiedField, copyText, handleCopyAWSExports }: any) {
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative">
        ...
        <div className="absolute right-2 top-3 flex items-center gap-1.5 text-slate-500">
          <button
            onClick={() => setVisiblePasswords((p: any) => ({ ...p, [secret.id!]: !p[secret.id!] }))}
...
function DatabaseCategoryDetails({ secret, isPassVisible, setVisiblePasswords, copiedId, copiedField, copyText, handleCopyDbUri }: any) {
  if (!secret.developerFields) return null;
  return (
...
function SshKeyCategoryDetails({ secret, isPassVisible, setVisiblePasswords, copiedId, copiedField, copyText }: any) {
  if (!secret.developerFields) return null;
  return (
    <div className="space-y-3">
      <div className="bg-[#121212] border border-white/5 p-2 rounded relative">
...
function ApiKeyCategoryDetails({ secret, copiedId, copiedField, copyText, handleDownloadDotenv }: any) {
  if (!secret.developerFields) return null;
  return (
    <div className="space-y-3">
...
function StandardCategoryDetails({ secret, isPassVisible, setVisiblePasswords, copiedId, copiedField, copyText }: any) {
  return (
    <div className="space-y-3">
      {secret.url && (
...
export function SecretInspector() {
  ...
  const secret = decryptedEntries.find((e: DecryptedSecret) => e.id === selectedEntryId);
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="space-y-3.5">
            ...
            {standardizedCategory === "AWS" && <AwsCategoryDetails secret={secret} isPassVisible={isPassVisible} setVisiblePasswords={setVisiblePasswords} copiedId={copiedId} copiedField={copiedField} copyText={copyText} handleCopyAWSExports={handleCopyAWSExports} />}
            {standardizedCategory === "Database" && <DatabaseCategoryDetails secret={secret} isPassVisible={isPassVisible} setVisiblePasswords={setVisiblePasswords} copiedId={copiedId} copiedField={copiedField} copyText={copyText} handleCopyDbUri={handleCopyDbUri} />}
            {standardizedCategory === "SSH Keys" && <SshKeyCategoryDetails secret={secret} isPassVisible={isPassVisible} setVisiblePasswords={setVisiblePasswords} copiedId={copiedId} copiedField={copiedField} copyText={copyText} />}
            {standardizedCategory === "API Keys" && <ApiKeyCategoryDetails secret={secret} copiedId={copiedId} copiedField={copiedField} copyText={copyText} handleDownloadDotenv={handleDownloadDotenv} />}
            {["Websites", "Emails", "Notes", "Servers"].includes(standardizedCategory) && <StandardCategoryDetails secret={secret} isPassVisible={isPassVisible} setVisiblePasswords={setVisiblePasswords} copiedId={copiedId} copiedField={copiedField} copyText={copyText} />}
