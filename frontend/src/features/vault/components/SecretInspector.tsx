...
export function SecretInspector() {
  ...
  if (!secret) {
    return (
      <div className="relative bg-[#0c0c0c] border border-white/5 rounded-lg p-12 text-center text-slate-500 font-mono text-xs">
        <EnhancedUIDecoration />

        <Cpu className="w-6 h-6 text-slate-800 mx-auto mb-2" />
        <span>
          Wybierz sekret z listy, aby wyświetlić szczegóły (Zero-Knowledge
          Audit)
        </span>
      </div>
    );
  ...
  const hasKey =
    secret.password || secret.developerFields?.awsSecretAccessKey || "";
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3 border-b border-white/5 pb-3">
            <div>
              <h3 className="font-display font-semibold text-white tracking-tight break-words">
                {secret.name}
              </h3>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              >
                <Star
                  className={`w-4 h-4 ${secret.isFavorite ? "fill-amber-400 text-amber-400" : ""}`}
                />
              </button>
          ...
          <div className="space-y-3.5">
            {secret.isLegacy && (
              <div className="bg-amber-950/10 border border-amber-900/30 p-3 rounded-lg text-amber-300 text-[10px] space-y-2">
                <p className="font-semibold">
                  ⚠️ Ten wpis jest przechowywany w czystym tekście (legacy).
                </p>
                <button
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-[#121212] border border-white/5 p-2 rounded">
                    ...
                    <span className="text-xs font-mono text-slate-300">
                      {secret.developerFields.region || "us-east-1"}
                    </span>
                  </div>
                  <div className="bg-[#121212] border border-white/5 p-2 rounded">
                    <span className="block text-[9px] font-mono text-slate-500 uppercase mb-0.5">
              ...
              <div className="space-y-2.5">
                <div className="grid grid-cols-3 gap-1.5">
                  <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-1">
                    ...
                    <span className="text-[11px] font-mono text-sky-400 font-bold">
                      {secret.developerFields.dbEngine || "PostgreSQL"}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-1">
                    ...
                    <span className="text-[11px] font-mono text-slate-300">
                      {secret.developerFields.dbPort || "5432"}
                    </span>
                  </div>
                  <div className="bg-[#121212] border border-white/5 p-1.5 rounded col-span-2">
                    <span className="block text-[8px] font-mono text-slate-500 uppercase">
                    ...
                    <span className="text-[11px] font-mono text-slate-300 truncate block select-all">
                      {secret.developerFields.dbName || "postgres"}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="bg-[#121212] border border-white/5 p-2 rounded">
                    ...
                    <span className="text-[11px] font-mono text-slate-300 truncate block select-all">
                      {secret.developerFields.dbUser || "Brak"}
                    </span>
                  </div>
                  <div className="bg-[#121212] border border-white/5 p-2 rounded relative">
                    ...
                    <span className="text-[11px] font-mono text-slate-300 truncate block">
                      {isPassVisible
                        ? secret.developerFields.dbPassword
                        : "••••••••••"}
                    </span>
                    <button
                      onClick={() =>
            ...
            {standardizedCategory === "SSH Keys" && secret.developerFields && (
              <div className="space-y-3">
                ...
                <div className="space-y-1 bg-[#070707] border border-white/5 p-2.5 rounded relative max-h-[160px] overflow-y-auto">
                  <span className="block text-[8px] font-mono text-slate-500 uppercase">
                    Klucz Prywatny RSA (Private Key)
                  </span>
                  <pre className="text-[9px] font-mono text-emerald-500/80 leading-normal select-all break-all whitespace-pre-wrap mt-1 select-all pr-6">
                    {secret.developerFields.sshPrivateKey
                      ? secret.developerFields.sshPrivateKey.substring(0, 70) +
                        "..."
                      : "Brak klucza"}
                  </pre>
                  {secret.developerFields.sshPrivateKey && (
            ...
            {["Websites", "Emails", "Notes", "Servers"].includes(
                ...
                {secret.url && (
                  <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative flex items-center justify-between">
                    <div className="min-w-0 pr-8">
                      ...
                      <span className="text-xs font-mono text-slate-300 truncate block select-all">
                        {secret.url}
                      </span>
                    </div>
            ...
            {secret.notes && (
              <div className="bg-[#121212] border border-white/5 p-3 rounded text-xs font-mono space-y-1">
                <span className="block text-[9px] text-slate-500 uppercase">
                  Dodatkowe notatki (Secure notes)
                </span>
