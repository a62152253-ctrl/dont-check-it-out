...
export const SecretHeader = ({ secret, toggleFavorite, handleEditClick }: Props) => (
  <div className="flex items-start justify-between gap-3 border-b border-white/5 pb-3">
    ...
    <div className="flex items-center gap-1 shrink-0">
      <button onClick={() => toggleFavorite(secret.id!, !!secret.isFavorite)} className="p-1.5 hover:bg-white/5 text-slate-500 hover:text-amber-400 rounded transition-all cursor-pointer" title="Do ulubionych">
        <Star className={`w-4 h-4 ${secret.isFavorite ? "fill-amber-400 text-amber-400" : ""}`} />
      </button>
      <button onClick={handleEditClick} className="p-1.5 hover:bg-white/5 text-slate-400 hover:text-white rounded transition-all cursor-pointer" title="Edytuj sekret">
