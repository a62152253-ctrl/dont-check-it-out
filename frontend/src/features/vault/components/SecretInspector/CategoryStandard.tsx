...
export const CategoryStandard = ({ secret, copiedId, copiedField, copyText, isPassVisible, setVisible }: Props) => (
  <div className="space-y-3">
    ...
    {secret.password && (
      <div className="bg-[#121212] border border-white/5 p-2.5 rounded relative">
        ...
        <span className="text-xs font-mono text-slate-200 select-all pr-8 break-all">{isPassVisible ? secret.password : "••••••••••••••••"}</span>
        <div className="absolute right-2 top-3 flex items-center gap-1.5 text-slate-500">
          <button onClick={() => setVisible(p => ({ ...p, [secret.id!]: !p[secret.id!] }))} className="hover:text-white cursor-pointer">
