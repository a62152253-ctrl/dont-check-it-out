...
export function PasswordGenerator() {
  ...
  const [genPreset, setGenPreset] = useState<
    "API_KEY" | "PASSWORD" | "PASSPHRASE" | "TOKEN"
  >("PASSWORD");
  ...
  return (
    <div className="relative bg-[#0c0c0c]/90 border border-white/10 rounded-2xl p-6 space-y-5 shadow-2xl relative overflow-hidden transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.02)]">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5">
            {(["PASSWORD", "PASSPHRASE", "API_KEY", "TOKEN"] as const).map(
              (pr) => (
                <button
                  key={pr}
                  type="button"
        ...
        {genPreset === "PASSPHRASE" ? (
          <div className="space-y-4 p-3.5 bg-white/[0.02] border border-white/5 rounded-lg">
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-slate-400 font-mono">
                <span>
                  {language === "PL" ? "Liczba słów:" : "Word count:"}
                </span>
                <span className={`${theme.text} font-bold`}>
                  {genPassphraseWords} słów
                </span>
              </div>
              <input
                type="range"
                ...
                onChange={(e) =>
                  setGenPassphraseWords(parseInt(e.target.value))
                }
          ...
          <div className="space-y-4 p-3.5 bg-white/[0.02] border border-white/5 rounded-lg">
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-slate-400 font-mono">
                <span>
                  {language === "PL" ? "Długość klucza:" : "Key length:"}
                </span>
                <span className={`${theme.text} font-bold`}>
                  {genLength} znaków
                </span>
        ...
        <div className="bg-[#121212] border border-white/5 rounded-lg p-3.5 space-y-2.5">
          <div className="flex justify-between items-center text-xs font-mono">
            <span className="text-slate-500">
              {language === "PL" ? "Entropia Hasła:" : "Key Entropy:"}
            </span>
            <span className={`${entropyDetails.color} font-bold`}>
              {entropyDetails.bits} bits
            </span>
          </div>
          ...
          <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
            <div
              className={`h-full transition-all duration-500 ${
                entropyDetails.bits >= 100
                  ? "bg-violet-500"
                  : entropyDetails.bits >= 75
                    ? "bg-emerald-500"
                    : entropyDetails.bits >= 50
                      ? "bg-amber-500"
                      : "bg-rose-500"
              }`}
              style={{
                width: `${Math.min(100, (entropyDetails.bits / 128) * 100)}%`,
              }}
            />
          </div>
          ...
          <div className="flex justify-between text-[10px] font-mono text-slate-400">
            <span>
              {language === "PL" ? "Ocena siły:" : "Strength rating:"}
            </span>
            <span className="font-semibold text-white">
              {entropyDetails.rating}
            </span>
          </div>
          <div className="flex justify-between text-[10px] font-mono text-slate-500">
            <span>
              {language === "PL"
                ? "Czas łamania (brute force):"
                : "Est. crack time (brute force):"}
            </span>
        ...
        >
          ...
          <span>
            {language === "PL"
              ? "Regeneruj Nowy Klucz"
              : "Regenerate New Dev Key"}
          </span>
        </button>
      </div>
    </div>
