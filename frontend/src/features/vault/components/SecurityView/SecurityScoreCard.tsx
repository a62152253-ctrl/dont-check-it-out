...
export function SecurityScoreCard() {
  ...
  return (
    <div className="relative bg-[#0c0c0c] border border-white/5 rounded-lg p-6 space-y-4 shadow-xl">
      <EnhancedUIDecoration />

      <div className="flex items-center gap-2.5 border-b border-white/5 pb-3">
        <Shield className="w-5 h-5 text-emerald-400" />
        <h3 className="text-sm font-semibold uppercase text-slate-300 font-mono">
          Centrum Bezpieczeństwa Sejfu / Vault Security Center
        </h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        <div className="md:col-span-4 text-center p-4 bg-[#121212] border border-white/5 rounded-lg">
          ...
          <div className="text-4xl font-light text-white my-2">
            {securityScore}%
          </div>
          ...
          <div className="flex justify-center gap-0.5 font-mono text-[9px] mb-2">
            {Array.from({ length: 10 }).map((_, i) => {
              ...
              return (
                <span
                  key={i}
                  className={isActive ? "text-emerald-400" : "text-slate-800"}
                >
                  █
                </span>
              );
        ...
        <div className="md:col-span-8 space-y-3 font-mono text-xs text-slate-400">
          <p className="leading-relaxed">
            Centrum Bezpieczeństwa przeprowadza w locie (zero-knowledge) analizę
            podatności Twoich haseł, kluczy API, tokenów oraz certyfikatów
            deweloperskich.
          </p>
          <p className="leading-relaxed">
            Wszystkie analizy wykonywane są w 100% lokalnie w przeglądarce za
            pomocą interfejsu WebCrypto API. Klucz szyfrowania nigdy nie
            opuszcza Twojej pamięci RAM.
          </p>
        </div>
      </div>
    </div>
