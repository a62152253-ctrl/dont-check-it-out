  ...
  const filteredDecryptedEntries = useMemo(() => {
    return decryptedEntries.filter((entry) => {
      ...
      const matchesSearch =
        ...
        (entry.environment &&
          entry.environment.toLowerCase().includes(queryLower));
    ...
    <div className="relative lg:col-span-5 space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-none">
        {[
          ...
          const isActive =
            viewMode === item.view && selectedCategory === item.cat;
          return (
            <button
              key={item.label}
              type="button"
      ...
      <div className="bg-[#0c0c0c] p-3 rounded-lg border border-white/5 flex flex-col sm:flex-row gap-3 items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-1.5 text-[9px] font-mono text-slate-400 shrink-0 bg-white/5 px-2 py-1.5 rounded border border-white/5">
          ...
          <kbd className="bg-black/40 px-1 py-0.5 rounded text-[8px] text-white">
            Ctrl+K
          </kbd>
          <span>Szukaj</span>
        </div>
        ...
        {isDecryptingEntries ? (
          <div className="bg-[#0c0c0c] border border-white/5 rounded-lg py-12 flex flex-col items-center justify-center text-slate-400">
            ...
            <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">
              WebCrypto AES-GCM deszyfracja w toku...
            </span>
          </div>
          ...
          <div className="bg-[#0c0c0c] border border-white/5 rounded-lg py-12 text-center text-slate-400">
            ...
            <p className="text-xs font-semibold">
              Brak sekretów pasujących do filtra
            </p>
            <p className="text-[10px] text-slate-500 mt-1">
              Stwórz nowy wpis deweloperski klikając "Dodaj sekret".
            </p>
          </div>
        ) : (
          filteredDecryptedEntries.map((secret) => {
            const isSelected = selectedEntryId === secret.id;
            const hasStrongValue =
              secret.password ||
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1 flex items-center gap-3">
                  <div
                    className={`w-8.5 h-8.5 rounded-lg flex items-center justify-center shrink-0 border transition-colors duration-200 ${
                      isSelected
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {secret.environment && (
                        <span
                          className={`text-[8px] font-mono px-1 py-0.5 rounded border uppercase font-semibold ${
                    ...
                    <div
                      className={`text-[9px] font-mono mt-1.5 space-y-0.5 ${isSelected ? "text-black/70" : "text-slate-500"}`}
                    >
                      ...
                      <div className="flex gap-2 flex-wrap items-center">
                        ...
                        <span className="flex items-center gap-1">
                          <span
                            className={`w-1 h-1 rounded-full ${recentlyOpenedSecrets.some((r) => r.id === secret.id) ? "bg-emerald-400 animate-pulse" : "bg-slate-600"}`}
                          />
                          {recentlyOpenedSecrets.some((r) => r.id === secret.id)
                            ? "Użyto: Dzisiaj"
                            : "Użyto: Niedawno"}
                        </span>
                      </div>
